...
static image<float> *smooth(image<float> *src, float sigma) {
  ...
  image<float> *tmp = new image<float>(src->height(), src->width(), src->depth(), false);
  image<float> *dst = new image<float>(src->width(), src->height(), src->depth(), false);
...
static image<float> *laplacian(image<float> *src) {
  ...
  int depth = src->depth();
  image<float> *dst = new image<float>(width, height, depth);  

  for (int z = 1; z < depth-1; z++) {
    for (int y = 1; y < height-1; y++) {
      for (int x = 1; x < width-1; x++) {
        float d2x = imRef(src, x-1, y, z) + imRef(src, x+1, y, z) -
	  2*imRef(src, x, y, z);
        float d2y = imRef(src, x, y-1, z) + imRef(src, x, y+1, z) -
	  2*imRef(src, x, y, z);
        imRef(dst, x, y, z) = d2x + d2y;
      }
    }
  }
  return dst;
